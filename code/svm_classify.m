...
train_num = size(train_image_feats, 1);%训练样本大小
test_num = size(test_image_feats, 1);%测试样本大小
categories = unique(train_labels);%类别
categories_num = length(categories);%类别数
W1 = zeros(categories_num, size(test_image_feats, 2));%存储支持向量机中的参数
B2 = zeros(categories_num, 1);%存储支持向量机的参数Wx+B
for i=1:categories_num%训练categories_num个支持向量机
    labels = ones(train_num,1).*-1;%初始化label的值
    labels(strcmp(categories{i}, train_labels)) = 1;%将第i类标签的值设置为1
    [W, B] = vl_svmtrain(train_image_feats', labels, 0.001);%svm训练0.01, 'MaxNumIterations', 100000
    W1(i,:) = W';
    B2(i) = B;
end

confidences = W1*test_image_feats'+repmat(B2,1,test_num);%利用categories_num个支持向量机查看测试样本属于各个类别的概率大小
[~, indices] = max(confidences);%找到各个样本最有可能的类别
predicted_categories = categories(indices);%将最可能的类别设置为预测值
end
