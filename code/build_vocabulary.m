...
descriptors_num = 8;% 我只挑选每张图片18个descriptor进行计算
descriptors = zeros(128, row * descriptors_num);
for i=1:row
    img = im2single(imread(image_paths{i}));
    [~,DESCRS] = vl_dsift(img);
    DESCRS = DESCRS(:,1:descriptors_num);
    descriptors(:,descriptors_num * (i-1) + 1 : descriptors_num * i) = DESCRS;% 生成所有图片的descriptors
end
    
[C, ~] = vl_kmeans(descriptors, vocab_size);%利用kmeans进行聚类，聚成vocab_size类
vocab = single(C');%一个vocab行，128列的vocabulary
